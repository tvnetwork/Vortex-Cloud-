    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 min-h-screen space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-border">
        <div className="space-y-1">
          ...
          <p className="text-text-secondary text-sm">Chat with other developers on the platform.</p>
        </div>
        <div className="flex items-center gap-2 text-sm text-text-secondary bg-card border border-border px-3 py-1.5 rounded-md">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[600px]">
        <div className="lg:col-span-3 bg-background border border-border rounded-[var(--radius-card)] flex flex-col overflow-hidden">
              ...
              <div className="space-y-6">
                {messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div className="h-8 w-8 rounded-[var(--radius-pill)] bg-card border border-border overflow-hidden shrink-0 mt-1">
                        {msg.senderPhoto ? (
                          <img src={msg.senderPhoto} alt="" className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                        ) : (
                          <Terminal className="h-full w-full p-2 text-muted" />
                        )}
                      </div>
                      ...
                      <div className={`space-y-1 ${isCurrentSender ? 'text-right' : ''}`}>
                        ...
                        <div className={`p-3 rounded-[var(--radius-card)] text-sm inline-block text-left break-words ${
                          isCurrentSender 
          ...
          {user ? (
            <form onSubmit={handleSendMessage} className="p-4 border-t border-border bg-surface flex gap-3">
              <input
                ...
                className="flex-1 bg-background border border-border rounded-md px-4 py-2 text-sm text-text-primary focus:outline-none focus:border-zinc-500"
              />
              <button
                type="submit"
                disabled={isSending || !newMessage.trim()}
                className="bg-primary text-text-primary hover:bg-secondary hover:bg-zinc-200 disabled:opacity-50 font-medium px-4 py-2 rounded-md transition-colors flex items-center justify-center gap-2"
              >
                Send
            ...
            <div className="p-4 text-center border-t border-border bg-surface text-sm text-muted">
              Sign in to send messages.
            </div>
          )}
        </div>

        <div className="bg-background border border-border rounded-[var(--radius-card)] overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-border bg-surface flex items-center gap-2">
            <Users className="h-4 w-4 text-text-secondary" />
            <h3 className="font-medium text-sm text-text-primary">Online ({onlineDevs.length})</h3>
          </div>

          <div className="p-4 overflow-y-auto flex-1 space-y-4">
            {onlineDevs.map((dev) => (
              <div key={dev.id} className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-[var(--radius-pill)] bg-card border border-border overflow-hidden shrink-0">
                  {dev.photoURL ? (
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-text-primary truncate">{dev.displayName}</p>
                  {dev.githubUsername && (
                    <p className="text-xs text-muted truncate">@{dev.githubUsername}</p>
                  )}
                </div>
              </div>
            ))}
